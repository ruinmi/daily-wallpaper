...
<template>
    <div>
        <p v-if="!work_done">
            坐着ing
            <n-progress :height="23" class="progress" type="line" :percentage="Number((work_count / work_duration * 100).toFixed(2))"
        ...
        <p v-else-if="work_done && !rest_done">
            休息ing
            <n-progress :height="23" class="progress" type="line" :percentage="Number((rest_count / rest_duration * 100).toFixed(2))"
                indicator-placement="inside" processing />
        </p>
        <p v-else>等待回来</p>
    </div>
